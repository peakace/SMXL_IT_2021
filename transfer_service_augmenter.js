...
var BIGQUERY_PROJECT_ID    = 'your_project_id';
var BIGQUERY_DATASET_ID    = 'your_dataset_id';
var SEND_ERROR_MESSAGES_TO = 'your_email_here';

var MAILGUN_URL            = 'optional mailgun setup';
var MAILGUN_AUTH           = 'optional mailgun setup';
var MAILGUN_FROM           = 'optional mailgun setup';

// END OF SETTINGS

var RECREATE_VIEWS         = false;
var CREATE_GEOTARGETS      = true;
var GEOTARGETS_URL         = 'https://developers.google.com/adwords/api/docs/appendix/geo/geotargets-2020-03-03.csv';
...
function extensions1( extensionType, accountIdForBigquery ){
	...
	var go = function( status1 ){
		...
		iteratorToList( selector.get() ).forEach( go2( status1 ) );
	};
	
	// no other way to get status from extensions :/
	[ 'REMOVED', 'ENABLED' ].forEach( go );
	
	var withoutStatus = go2();
	...
	iteratorToList(
		...
		// due to 50k limit it is possible that this item is not in siteLinksMap
		if( siteLinksMap[ item.getId() ] ){
...
function jobIdToStatus( jobId ){
	if( jobId.lastIndexOf( '.' ) >= 0 ){
		// Invalid job ID "biddy-io:US.job_asdfasdfasdfasdf-asdfasdfasdf"
...
function sendEmail( to, subject, text ){
	if( type of to != 'string' || to.indexOf( '@' ) == -1 ){
		Logger.log( 'no email address for notiications' );
		return;
	}
	if( !text ){
